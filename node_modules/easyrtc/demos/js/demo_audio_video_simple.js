...
var delayStart = new Date().getTime() / 1000;
...
function draw(v,c,w,h) {  
   ...
   var avgDenom = 0;
   for (var w1 = 0; w1 < width/8; w1++) {
    for (var h1 = 7*height/8; h1 < height; h1++) {
      avgDenom++;
      var i = h1*width + w1;
      
     if (lastScreen != null) {
          lastBrightness += 0.34 * lastScreen.data[i] + 0.5 * lastScreen.data[i + 1] + 0.16 * lastScreen.data[i + 2];
          currentBrightness += 0.34 * currentScreen.data[i] + 0.5 * currentScreen.data[i + 1] + 0.16 * currentScreen.data[i + 2];          
      }
  ...
  lastBrightness = lastBrightness/avgDenom;
  currentBrightness = currentBrightness/avgDenom;
  //console.log("Time difference:" + (now - delayStart));
  if (now - delayStart > 1) {
    ...
    if (motionDetected) {
      console.log("Time:"+(now - delayStart))
      if (now - delayStart < 1) {
        if (!(Math.abs(baseline-currentBrightness) < 3)) {
          console.log("The baseline diff is:"+ Math.abs(baseline-currentBrightness))
          motionDetected = false;
          delayStart = new Date().getTime() / 1000;
        }
      }
      else {
        console.log("Motion detected. Value:"+Math.abs(baseline-currentBrightness));
        motionDetected = false;
        delayStart = new Date().getTime() / 1000;
        //DO action
      }
      ...
      if (Math.abs(lastBrightness-currentBrightness)> 0.5) {
        console.log(Math.abs(lastBrightness-currentBrightness));
        delayStart = new Date().getTime() / 1000;
        console.log("RESET DELAY");
        motionDetected = true;
        baseline = currentBrightness;
        //console.log("Motion detected. Value:"+Math.abs(lastBrightness-currentBrightness));
...
buttonContext.globalAlpha = 0.001;
buttonContext.drawImage(placeholder, 0, 0, 930, 800); 

/*Adding comment so there is a change*/
canvas.height = ch;  
var call = true;
